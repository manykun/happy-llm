# -*- coding: utf-8 -*-
"""Tiny-LLM 全参数 SFT 脚本，支持 PyTorch DDP。

两卡 4090 推荐启动方式：
    CUDA_VISIBLE_DEVICES=0,1 torchrun --standalone --nproc_per_node=2 ddp_sft_full.py

SFT 与预训练的 DDP 逻辑完全相同：每张 GPU 一个进程、每个进程读不同数据、
backward 时自动同步梯度；区别只在数据集和预训练权重加载。
"""
...
def train_epoch(epoch: int) -> None:
    ...
    for step, (x, y, loss_mask) in enumerate(train_loader):
        ...
        if (step + 1) % args.save_interval == 0:
            ckp = os.path.join(
                args.save_dir,
                f"sft_dim{lm_config.dim}_layers{lm_config.n_layers}_vocab_size{lm_config.vocab_size}.pth",
            )
        ...
        if (step + 1) % 20000 == 0:
            ckp = os.path.join(
                args.save_dir,
                f"sft_dim{lm_config.dim}_layers{lm_config.n_layers}_vocab_size{lm_config.vocab_size}_step{step + 1}.pth",
            )
            save_checkpoint(model, ckp, ddp_ctx)
...
def load_pretrained_weights(model: torch.nn.Module) -> None:
    ...
    权重先加载到 CPU，再由 wrap_model_for_ddp 移动到当前 GPU，可以避免所有进程
    同时把 checkpoint 读到 cuda:0 上。
    """

    if not os.path.exists(args.pretrained_model_path):
        raise FileNotFoundError(f"找不到预训练权重：{args.pretrained_model_path}")

    state_dict = torch.load(args.pretrained_model_path, map_location="cpu")
    for unwanted_prefix in ("_orig_mod.", "module."):
    ...
    logger(
        f"已加载预训练权重：{args.pretrained_model_path} "
        f"(missing={len(missing_keys)}, unexpected={len(unexpected_keys)})"
    )


def init_model() -> tuple[torch.nn.Module, AutoTokenizer]:
    """初始化 tokenizer、模型并加载预训练权重。"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tiny-LLM DDP Full SFT")
    ...
    parser.add_argument("--data_path", type=str, default="./BelleGroup_sft.jsonl", help="SFT 数据路径")
    parser.add_argument("--tokenizer_path", type=str, default="./tokenizer_k/", help="tokenizer 路径")
    parser.add_argument(
        "--pretrained_model_path",
        type=str,
        default="./base_model_215M/pretrain_1024_18_6144.pth",
        help="预训练权重路径",
    )
...
if __name__ == "__main__":
    ...
    if args.use_swanlab:
        ...
        if ddp_ctx.is_main_process:
            swanlab.init(project="Happy-LLM", experiment_name="SFT-215M-DDP", config=vars(args))
